...
public class VariableImp {
    ...
    @Step({"Store txt <file name>'s value from classpath with <key> during scenario",
            "Senaryo boyunca txt <dosya adı> içeriğini <key> anahtarı ile sakla"})
    public void storeTxtFileValueDuringScenario(String fileName, String key) {
        FileHelper fileHelper = new FileHelper();
        String filePath = Objects.requireNonNull(getClass().getClassLoader().getResource(fileName)).getPath();
        String value = fileHelper.readFileAsString(filePath); // Dosyayı düz metin olarak oku

        ScenarioDataStore.put(key, value); // JSON değil düz metin olarak sakla
        log.info(LOG_INFO, key, fileName); // Log ile saklama işlemini kaydet
    }
